...
if ($f == 'chat') {
    ...
    if ($s == 'send_message') {
         ...
         //TODO: ЗАКРЫВАЕМ ВОЗМОЖНОСТЬ ОТПРАВЛЯТЬ СООБЩЕНИЕ ЕСЛИ ГРУППА ЗАКРЫТА
        if (Is_GroupChatClosed($_GET['group_id'])) {
            $data['status'] = 401;
            $data['error'] = "group is closed";
            header("Content-type: application/json");
            echo json_encode($data);
            exit();
        }
                ...
                if (!empty($_POST['user_id']) && Wo_CheckMainSession($hash_id) === true) {
                    ...
                    if ($messages > 0) {
                        ...
                        if ($wo['config']['emailNotification'] == 1) {
                            ...
                            if (!in_array(false, $send_notif) || 1) {
...
//ЕСЛИ ЧАТ МЕРОПРИЯТИЯ ТО ЕГО УДАЛИТЬ НЕЛЬЗЯ, ЧАТ ЗАКРЫВАЕТСЯ
        if (IsEventChatGroup($_GET['group_id'])) {
            $result = Wo_CloseEventChatGroup($_GET['group_id']);
            $data['status'] = 304;
            if ($result === true) {
                $data['status'] = 200;
            }
