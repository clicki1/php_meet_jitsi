...
//TODO: функция интеллектуального поиска людей
function Wo_GetSearchUsersByMyTags($user_id) {
    global $db;
    $data = array();
    $u_tag_fs = $db->where("user_id", (int) $user_id)->get(T_USER_FIND_TAGS);

    if (count($u_tag_fs) > 0) {

        foreach ($u_tag_fs as $tag) {

            $u_tags = $db->where("tag_id", $tag->tag_id)->where('user_id', $user_id, "<>")->get(T_USER_TAGS);

            if (count($u_tags) > 0) {
                foreach ($u_tags as $u_tag) {
                    $u_id = $u_tag->user_id;
...
//TODO: переработанный фильтр пользователей, добавлен фильтр по городу, сфера деятельности, спецификации
// и подбор от системы
function Wo_GetSearchFilter($result, $limit = 30, $offset = 0) {
    ...
    if ($wo['loggedin'] !== false && !empty($result)) {

        if (isset($result['my_find']) && !empty($result['my_find']) && ($result['my_find'] == 'all')) {
            $result = [];
            $result['my_find'] = 'all';
        ...
        // return  Wo_GetSearchUsersByMyTags($wo['user']['id']);
    }
    ...
    if (!empty($result['city_id'])) {
        $city_id = Wo_Secure($result['city_id']);
    }
    if (!empty($result['category_id'])) {
        $category_id = Wo_Secure($result['category_id']);
    }
    if (!empty($result['tag'])) {
        $tags = $result['tag'];
    }
    ...
    if (!empty($result['city_id'])) {
        if ($result['city_id'] != 'all') {
            $query .= " AND (`city_id` = '{$city_id}')";
        }
    }

    if (!empty($result['category_id'])) {
        if ($result['category_id'] != 'all') {
...
//УЧИТЫВАЕМ ТЕГИ
    if (isset($tags) && count($tags) > 0) {
        ...
        foreach ($tags as $tag) {
            ...
            foreach ($data as $user) {
                ...
                $res = $db->where("user_id", (int) $user['id'])->where("tag_id", $tag)->get(T_USER_TAGS);
                if (count($res) > 0 && !isset($data_with_tags[$user['id']])) {
                    $user_id = (string) $user['id'];
                    $data_with_tags[$user_id] = $user;
                }
            }
        }
        $data = $data_with_tags;
    ...
    if ($wo['loggedin'] !== false && isset($result['my_find']) && !empty($result['my_find']) && ($result['my_find'] == 'all')) {
        $data_my = [];
        $users_my = Wo_GetSearchUsersByMyTags($wo['user']['id']);

        foreach ($data as $user) {
            if (isset($users_my[$user['id']])) {

                $user_id = (string) $user['id'];
                $data_my[$user_id] = $user;
            }
        }
...
function Wo_ProfileCompletion() {
    ...
    if (Wo_GetUserFindTagsCount($wo['user']['id'])) {
        $data[3] = 20;
    }
//    if (!empty($wo['user']['country_id'])) {
//        $data[4] = 20;
//    }
    if (!empty($wo['user']['city_id'])) {
        $data[4] = 20;
    }
    if (!empty($wo['user']['category_id'])) {
        $data[5] = 20;
    }
