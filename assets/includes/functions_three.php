...
//TODO ИЗМЕНИЛ ЛОГИКУ ПОИСКА ПОЛЬЗОВАТЕЛЕЙ ПРИ ОТПРАВКЕ ДЕНЕГ
...
//    $sql = "SELECT `user_id` FROM " . T_USERS . " WHERE `user_id` <> {$user} AND `username`  LIKE '%$name%' {$sub_sql} $limit_text";
    $sql = "SELECT `user_id` FROM " . T_USERS . " WHERE ((`username` LIKE '%$name%' {$sub_sql}) OR CONCAT ( `first_name`,  ' ', `last_name` ) LIKE '%$name%') AND `active` = '1' $limit_text";
...
//TODO УДАЛЕНИЕ ЧАТА ИЗЕМЕНЕНИЕ ЛОГИКИ
...
//ВОЗВРАТ ДЕНЕГ НА СЧЕТ УЧАСТНКА МЕТОПРИЯТИЯ, ЕСЛИ ОН ИЗ НЕГО ВЫШЕЛ
function Wo_RepaymentEventUsers($event_id, $repayment_cf = 1, $admin_id = 1) {
    global $sqlConnect, $wo, $db;
    $user_id = $wo["user"]["id"];
    $user_wallet = $wo["user"]["wallet"];
    Wo_UpdateEventActive($event_id);
    //ВОЗВРАТ ДЕНЕГ НА СЧЕТ УЧАСТНКА МЕТОПРИЯТИЯ, ЕСЛИ ОН ИЗ НЕГО ВЫШЕЛ
    $sql = "SELECT * FROM " . T_EVENTS . "  WHERE `id` = '$event_id ' ";
    $query = mysqli_query($sqlConnect, $sql);
    //  $data = array();
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $event_price = $fetched_data['event_price'];

        $event_wallet = $fetched_data['event_wallet'];
        //   $data = $fetched_data;
        if ($fetched_data['event_active'] > 0) {
            return false;
        }
    }

    $repayment = (int) $event_price * $repayment_cf;
    $update_wallet = ((int) $user_wallet + (int) $repayment);
    $update_event_wallet = ((int) $event_wallet - (int) $repayment);
    $result = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `wallet` = " . $update_wallet . " WHERE `user_id` = '" . $user_id . "'");
    $result_event = mysqli_query($sqlConnect, "UPDATE " . T_EVENTS . " SET `event_wallet` = " . $update_event_wallet . " WHERE `id` = '" . $event_id . "'");
    //ВОЗВРАТ
    //ВНОСИМ ИЗМЕНЕНИЯ О ВОЗВРАТЕ В СИСТЕМНЫЙ КОШЕЛЕК
    Wo_AddPayForSysWallet($event_id, "event_going_out", (int) $event_price, $repayment_cf);

    return true;
}

//СНЯТИЕ ДЕНЕГ СО СЧЕТА УЧАСТНИКА МЕРОПРИЯТИЯ
function Wo_PaymentEventUsers($event_id) {
    global $sqlConnect, $wo;
    $user_id = $wo["user"]["id"];
    $user_wallet = $wo["user"]["wallet"];
    //СНЯТИЕ ДЕНЕГ СО СЧЕТА УЧАСТНИКА МЕРОПРИЯТИЯ
    ...
    $update_wallet = (int) $user_wallet - (int) $data['event_price'];
    $update_event_wallet = (int) $data['event_price'] + (int) $data['event_wallet'];

    if ($update_wallet < 0) {
        return false;
    }

    $result_user = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `wallet` = " . $update_wallet . " WHERE `user_id` = '" . $user_id . "'");
    $result_event = mysqli_query($sqlConnect, "UPDATE " . T_EVENTS . " SET `event_wallet` = " . $update_event_wallet . " WHERE `id` = '" . $event_id . "'");
    //СНЯТИЕ
    //ДОБАВЛЯЕМ ДАННЫЕ В СИСТЕМНЫЙ КОШЕЛЕК
    Wo_AddPayForSysWallet($event_id, "event_going_in", (int) $data['event_price']);
    return true;
}

//СПИСОК ВСЕХ КАТЕГОРИЙ
function Wo_GetEventsCategories() {
    global $db;
    $categories = $db->get(T_EVENTS_CAT);
    return $categories;
}

//ДОБАВЛЯЕТ ТЭГИ К МЕРОПРИЯТИЯМ
function Wo_AddTagsEvents($event_id, $tags) {
    global $db;
    $tags_array = explode(",", $tags);
    foreach ($tags_array as $tag) {
        $db_tag = $db->where('tag', $tag)->getOne(T_HASHTAGS, [
            'id'
        ]);
        if ($db_tag) {
            $db_etag = $db->where('event_id', $event_id)->where('tag_id', $db_tag->id)->getOne(T_EVENTS_TAGS, [
                'id'
            ]);

            if (empty($db_etag)) {
                //XXX ДОБАВИТЬ ДОБАВЛЕНИЕ ВЕСА ТЭГА
                //TODO ДОБАВИТЬ УВЕЛИЧЕНИЕ ВЕСА ТЭГА, 
                $tag_all = $db->where('tag_id', $db_tag->id)->get(T_EVENTS_TAGS);
                $count_add = count($tag_all) + 1;
                $db->where('id', $db_tag->id)->update(T_HASHTAGS, [
                    'trend_use_num' => $count_add,
                ]);
                $res = $db->insert(T_EVENTS_TAGS, [
                    'event_id' => $event_id,
                    'tag_id' => $db_tag->id
                ]);
            }
            ...
            $new_tag_id = $db->insert(T_HASHTAGS, [
                'hash' => md5($tag),
                'tag' => $tag,
...
//ВЫВОДИТ ТЭГИ МЕРОПРИЯТИЯ
function Wo_GetEventTags($event_id) {
    global $db;
    $tags_str = '';
    $db_etags = $db->where('event_id', $event_id)->get(T_EVENTS_TAGS);
    foreach ($db_etags as $tag) {
        $str = $db->where('id', $tag->tag_id)->getOne(T_HASHTAGS, [
            'tag'
        ]);
        $tags_str .= $str->tag . ',';
    }
    $str = substr($tags_str, 0, strlen($tags_str) - 1);
    return $str;
}

//ОБНОВЛЯЕТ ТЭГТ МЕРОПРИТИЯ
function Wo_UpdateTagsEvents($event_id, $tags) {
    global $db;
    $db->where('event_id', $event_id)->delete(T_EVENTS_TAGS);
    Wo_AddTagsEvents($event_id, $tags);
}

//ВАЛИДАЦИЯ ДЛЯ ОПЛАТЫ СОЗДАНИЯ МЕРОПРИТИЯ (ПРИ СОЗДАНИИ НА СЧЕТУ ДОЛЖНО БЫТЬ МНИМУМ 1000 РУБЛЕЙ)

function Wo_ValidationEventPayUser($event_id, $event_create_coats) {
    global $db, $wo;
     $event = $db->where('id', $event_id)->getOne(T_EVENTS);
    $user_wallet = $wo['user']['wallet'] + (int)$event->event_create_price;
    return ($user_wallet - $event_create_coats) >= 0 ? false : true;
}

//СНЯТИЕ 1000 РУБЛЕЙ СО СЧЕТА ПРИ СОЗДАНИИ МЕРОПРИЯТИЯ
function Wo_EventPayUser($coast, $event_id) {
    global $db, $wo;

    $db->where('user_id', $wo["user"]["id"])->update(T_USERS, array(
        'wallet' => ((int) $wo["user"]["wallet"] - (int) $coast)
    ));
    $db->where('id', $event_id)->update(T_EVENTS, [
        'event_wallet' => (int) $coast
    ]);
...
//ВАЛИДАЦИЯ ПО ДАТЕ И ВРЕМЕНИ СОЗДАНИЯ И ИЗМЕНЕНИЯ МЕРОПРИЯТИЯ
function Wo_ValidationEventDateStart($date_start, $date_end, $time_start, $time_end) {
    $d_start = explode('-', $date_start);
    $d_end = explode('-', $date_end);
    $t_start = explode(':', $time_start);
    $t_end = explode(':', $time_end);

    $start = mktime((int) $t_start[0], (int) $t_start[1], 0, (int) $d_start[1], (int) $d_start[2], (int) $d_start[0]);
    $end = mktime((int) $t_end[0], (int) $t_end[1], 0, (int) $d_end[1], (int) $d_end[2], (int) $d_end[0]);

    return (($end - $start) > 0 ? false : true);
}

//ФИЛЬТР МЕРОПРИЯТИЙ ПО КАТЕГОРИЯМ И ТИПУ
function Wo_GetEventsSearch($caregory_id = 0, $type_id = "all") {
    global $wo, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($type_id == 'my') {
        $db->where('poster_id', $wo['user']['id']);
    }
    //ПОНРАВИВШИЕСЯ МЕРОПРИЯТИЯ
    if ($type_id == 'like' || $type_id == 'going' || $type_id == 'invite') {

        if ($type_id == 'like') {
            $e_ints = $db->where("user_id", $wo['user']['id'])->get(T_EVENTS_INT);
        }

        if ($type_id == 'going') {
            $e_ints = $db->where("user_id", $wo['user']['id'])->get(T_EVENTS_GOING);
        }

        if ($type_id == 'invite') {
            $e_ints = $db->where("invited_id", $wo['user']['id'])->get(T_EVENTS_INV);
        }

        foreach ($e_ints as $e_int) {
            if ((int) $caregory_id) {
                $db->where("category_id", $caregory_id);
            }
            $events = $db->where("id", $e_int->event_id)->get(T_EVENTS);
            foreach ($events as $fetched_data) {
                $fetched_data = (array) $fetched_data;
                $fetched_data['user_data'] = Wo_UserData($fetched_data['poster_id']);
                $fetched_data['cover'] = Wo_GetMedia($fetched_data['cover']);
                $fetched_data['is_owner'] = Is_EventOwner($fetched_data['id']);
                $fetched_data['url'] = Wo_SeoLink("index.php?link1=show-event&eid=" . $fetched_data['id']);
        ...
        $events = $db->get(T_EVENTS);
        foreach ($events as $fetched_data) {
            $fetched_data = (array) $fetched_data;

            if ($type_id == 'old') {
                $d_end = explode('-', $fetched_data['end_date']);
                $t_end = explode(':', $fetched_data['end_time']);
                $end = mktime((int) $t_end[0], (int) $t_end[1], 0, (int) $d_end[1], (int) $d_end[2], (int) $d_end[0]);
                if (time() < $end) {
                    continue;
                }
...
//СОЗДАЕМ ГРУППОВОЙ ЧАТ ПРИ СОЗДАНИИ МЕРОПРИЯТИЯ
function Wo_AddGroupChatForEvent($event_id) {
    global $wo, $db;
    $event = $db->where("id", $event_id)->get(T_EVENTS);
    //return count($event);
    if (count($event)) {
        $g_chat_id = $db->insert(T_GROUP_CHAT, [
            'user_id' => $wo['user']['id'],
            'group_name' => $event[0]->name,
            'event_id' => $event[0]->id,
            'time' => time(),
        ]);

        $g_chat_user_id = $db->insert(T_GROUP_CHAT_USERS, [
            'user_id' => $wo['user']['id'],
            'group_id' => $g_chat_id,
            'event_id' => $event[0]->id,
            'last_seen' => time(),
...
//ОБНОВЛЯЕМ ИМЯ ЧАТА
function Wo_UpdateGroupChatForEvent($event_id, $event_name) {
    global $wo, $db;

    $chat = $db->where("event_id", $event_id)->get(T_GROUP_CHAT);

    if (count($chat)) {
        $db->where('event_id', $event_id)->update(T_GROUP_CHAT, array(
...
//ДОБАВЛЯЕМ ИЛИ УДАЛЯЕМ ПОЛЬЗОВАТЕЛЯ ЧАТА
function Wo_AddMemberGroupChat($event_id) {
    global $wo, $db;

    $chat_mb = $db->where("event_id", $event_id)->where("user_id", $wo['user']['id'])->get(T_GROUP_CHAT_USERS);
    ...
    if (count($chat_mb)) {
        ...
        if (count($chat)) {
            ...
            $res = $db->where('event_id', $event_id)->where("user_id", $wo['user']['id'])->delete(T_GROUP_CHAT_USERS);
        }
    } else {
        $chat = $db->where("event_id", $event_id)->get(T_GROUP_CHAT);
        ...
        if (count($chat)) {
            ...
            $res = $db->insert(T_GROUP_CHAT_USERS, [
                'user_id' => $wo['user']['id'],
...
//ПРОВЕРЯЕМ ВСТУПИЛ ЛИ В МЕРОПРИЯТИЕ ПОЛЬЗОВАТЕЛЬ ИЛИ НЕТ
function Is_EventGoungUser($id, $user = false, $admin = true) {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
    ...
    $result = $db->where("event_id", $id)->where("user_id", (int) $user)->get(T_EVENTS_GOING);

    return count($result) ? true : false;
}

//УДАЛЯМ ЧАТ ПРИ УДАЛЕНИИ МЕРОПРИЯТИЯ
function Wo_DeleteEventGroupChat($event_id) {
    global $wo, $db;

    if ($wo['loggedin'] == false) {
        return false;
    }
    if (!Is_EventOwner($event_id)) {
        if (Wo_IsAdmin() == false && Wo_IsModerator() == false) {
            return false;
        }
    }

    $group = $db->where("event_id", $event_id)->get(T_GROUP_CHAT);

    if (count($group)) {
        $group_chat_user = $db->where('group_id', $group[0]->group_id)->get(T_GROUP_CHAT_USERS);
        $group_chat_messages = $db->where('group_id', $group[0]->group_id)->get(T_MESSAGES);

        if (count($group_chat_user)) {
            $db->where('group_id', $group[0]->group_id)->delete(T_GROUP_CHAT_USERS);
        }

        if (count($group_chat_messages)) {
            $db->where('group_id', $group[0]->group_id)->delete(T_MESSAGES);
        }
        $db->where("event_id", $event_id)->delete(T_GROUP_CHAT);
    }

    return true;
}

//ПРОВЕРЯЕМ СОЗДАВАЛСЯ ЛИ ГРУППОВОЙ ЧАТ
function Is_IssetEventGroupChat($group_id) {
    global $wo, $db;

    $group = $db->where("group_id", $group_id)->get(T_GROUP_CHAT);

    return (empty($group[0]->event_id)) ? false : true;
}

//ПОЛУЧАЕМ ДАННЫЕ ГРУПОВОГО ЧАТА ДЛЯ МЕРОПРИЯТИЯ
function Wo_GroupChatEvent($event_id) {
    global $wo, $sqlConnect, $db;

    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_GROUP_CHAT . "  WHERE `event_id` = '$event_id'");
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if (!empty($fetched_data)) {
...
function Wo_DateToTimestamp($date, $time) {
    $dt = explode('-', $date);

    $tt = explode(':', $time);

    return mktime((int) $tt[0], (int) $tt[1], 0, (int) $dt[1], (int) $dt[2], (int) $dt[0]);
}

//ОБНОВЛЯЕМ АКТИВНОСТЬ МЕРОПРИЯТИЯ ПО ДАТЕ
function Wo_UpdateEventActive($event_id = null) {
    global $wo, $db;

    if ($event_id) {
        $db->where("id", (int) $event_id);
    }
    $events = $db->where("event_active", Array(0, 1), 'BETWEEN')->get(T_EVENTS);

    if (count($events) > 0) {

        foreach ($events as $event) {
            //TODO отправка сообщения за 1 час
            if ($event->start_timestamp - time() < 3600 && empty($event->notification1)) {
                Wo_EventGroupChat_Message($event->id, $event->poster_id, $messages = 'Мероприятие начнется через 1 час');
                $db->where("id", $event->id)->update(T_EVENTS, [
                    'notification1' => time()
                ]);
                //ОТПРАВЛЯЕМ ПИСЬМО ВСЕМ УЧАСТНИКАМ И ОРГАНИЗАТОРУ
                Wo_EventSendMail($event->id, $messages);
            }
            //TODO отправка сообщения за 15 минут
            if ($event->start_timestamp - time() < 900 && empty($event->notification2)) {
                Wo_EventGroupChat_Message($event->id, $event->poster_id, $messages = 'Мероприятие начнется через 15 минут');
                $db->where("id", $event->id)->update(T_EVENTS, [
                    'notification2' => time()
                ]);
            }
            //TODO отправка сообщения когда мероприятие началось
            if ($event->start_timestamp - time() < 1 && empty($event->notification_start)) {
                Wo_EventGroupChat_Message($event->id, $event->poster_id, $messages = 'Мероприятие началось! Внимание! Если окно транслиции не открылось, обновите страницу мероприятия');
                $db->where("id", $event->id)->update(T_EVENTS, [
                    'notification_start' => time()
                ]);
            }

//        Wo_EventGroupChat_Message($event_id, $poster_id, $messages = '');

            if ($event->start_timestamp > time() && $event->event_active != 0 && $event->event_active != 3) {
                $db->where("id", $event->id)->update(T_EVENTS, [
                    'event_active' => 0
                ]);
            } elseif ($event->start_timestamp < time() && $event->end_timestamp > time() && $event->event_active != 1 && $event->event_active != 3) {
                $db->where("id", $event->id)->update(T_EVENTS, [
                    'event_active' => 1
                ]);
            } elseif ($event->end_timestamp < time() && $event->event_active != 2 && $event->event_active != 3) {
                //МЕРОПРИЯТИЕ ОКОНЧЕНО
                $db->where("id", $event->id)->update(T_EVENTS, [
                    'event_active' => 2
                ]);
                //TODO МЕРОПРИЯТИЕ ПРОШЛО РАСЧЕТ ДЕНЕЖНЫХ СТРЕДСТВ
                Wo_AddPayForSysWallet($event->id, 'event_end', 0, 0.7);
            }
        }
    };
...
//ОТМЕНА МЕРОПРИЯТИЯ
function Wo_DeleteEvent_Cansel($id = false, $desctiption = '') {
    global $sqlConnect, $wo, $db;
    $result = false;
    $poser_repayment = 500;
    if ($wo['loggedin'] == false) {
    ...
    $event = $db->where("id", $id)->get(T_EVENTS);
    //ПОЛУЧАЕМ СПИСОК УЧАСТНИКОВ МЕРОПРИЯТИЯ
    $events_g = $db->where("event_id", $id)->get(T_EVENTS_GOING);
    if (count($events_g)) {
        //
        foreach ($events_g as $eg) {
            $user_g = $db->where("user_id", $eg->user_id)->getOne(T_USERS);
//ВОЗВРАЩАЕМ ДЕНЬГИ ЗА МЕРОПРИЯТИЕ
            $db->where("user_id", $eg->user_id)->update(T_USERS, [
                'wallet' => ((int) $user_g->wallet + (int) $event[0]->event_price)
            ]);
//СНИМАЕМ С КОШЕЛЬКА МЕРОПРИЯТИЯ ДЕНЬГИ
            $db->where("id", $id)->update(T_EVENTS, [
                'event_wallet' => ((int) $event[0]->event_wallet - (int) $event[0]->event_price)
            ]);
        }
    }
    //ПОЛУЧАЕМ МЕРОПРИЯТИЕ
    $event = $db->where("id", $id)->get(T_EVENTS);
//ПОЛУЧАЕМ ДАННЫЕ ОРГАНИЗАТОРА МЕРОПРИЯТИЯ
    $user_p = $db->where("user_id", $event[0]->poster_id)->getOne(T_USERS);
//ВОЗВРАЩАЕМ ДЕНЬГИ ОРГАНИЗАТОРУ МЕРОПРИЯТИЯ, ЗА МИНУСОМ КОММИСИИ
    $db->where("user_id", $user_p->user_id)->update(T_USERS, [
        'wallet' => ((int) $user_p->wallet + $poser_repayment)
    ]);
//СНИМАЕМ С КОШЕЛЬКА МЕРОПРИЯТИЯ ДЕНЬГИ КОТОРЫЕ ВЕРНУЛИ ОРГАНИЗАТОРУ
    $db->where("id", $id)->update(T_EVENTS, [
        'event_wallet' => ((int) $event[0]->event_wallet - $poser_repayment)
    ]);
//УДАЛЯЕМ ДАННЫЕ О МЕРОПРИЯТИИ (ИВАЙТЫ, ВСТУПИВШИЕ, ИЗБРАННОЕ, СОБЫТИЯ, ПОСТЫ)
    $result = mysqli_query($sqlConnect, "DELETE FROM " . T_EVENTS_INT . "  WHERE `event_id` = '$id'");
    $result = mysqli_query($sqlConnect, "DELETE FROM " . T_EVENTS_GOING . "  WHERE `event_id` = '$id'");
    $result = mysqli_query($sqlConnect, "DELETE FROM " . T_EVENTS_INV . "  WHERE `event_id` = '$id'");
    $result = mysqli_query($sqlConnect, "DELETE FROM " . T_NOTIFICATION . " WHERE `event_id` = {$id}");
...
//ВНИСИМ ДАННЫЕ О ТОМ ЧТО МЕРОПРИЯТИЕ ОТМЕНЕНО
    $db->where("id", $id)->update(T_EVENTS, [
        'event_delete' => time(),
        'event_delete_desc' => $desctiption,
        'event_active' => 3,
    ]);
    //ИЗМЕНЯЕМ НАЗВАНИЕ ЧАТА МЕРОПРИЯТИЯ
    Wo_DeleteEventGroupChat_Cansel($id, $event[0]->poster_id);
    //ОПОВЕЗАЕМ УЧАСТНИКОВ О ТОМ ЧТО МЕРОПРИЯТИЕ УДАЛЕНО И ДЕНЬГИ ВОЗВРАЩЕНЫ
    return true;
}

//ВНОСИМ ИЗМЕНЕНИЕ В ЧАТ ПРИ ОТМЕНЕ МЕРОПРИЯТИЯ
function Wo_DeleteEventGroupChat_Cansel($event_id, $poster_id) {
    global $wo, $db;
    ...
    if (count($group)) {
        ...
        $db->where("event_id", $event_id)->update(T_GROUP_CHAT, [
            'group_name' => $group[0]->group_name . ' (МЕРОПРИЯТИЕ ОТМЕНЕНО)'
        ]);

        $db->insert(T_MESSAGES, [
            'from_id' => $poster_id,
            'group_id' => $group[0]->group_id,
            'text' => $wo['lang']['event_del_notification'],
        ]);
    }
...
//УВЕДОМЛЕНИЕ В ЧАТ МЕРОПРИЯТИЯ
function Wo_EventGroupChat_Message($event_id, $poster_id, $messages = '') {
    global $wo, $db;
...
//ДОБАВЛЯЕМ СФЕРЫ ДЕЯТЕЛЬНОСТИ МЕРОПРИЯТИЙ
function Wo_AddUsersCategories() {
    global $wo, $db;
    ...
    foreach ($categories as $category) {
        ...
        $db->insert(T_USER_CATEGORIES, [
            'name' => $category
        ]);
    }

    return $db->get(T_USER_CATEGORIES);
}

//ДОБАВЛЯЕМ СПЕЦИАЛИЗАЦИЮ МЕРОПРИЯТИЙ ПО СФЕРЕ ДЕЯТЕЛЬНОСТИ
function Wo_AddUsersCategoriesTags() {
    global $wo, $db;
    ...
    $tags = array(
        ...
        "15*Алкогольные напитки (продвижение, оптовая торговля)",
        "15*Безалкогольные напитки (продвижение, оптовая торговля)",
        "15*Безалкогольные напитки (производство)",
        "15*Вино (производство)",
        "15*Детское питание (продвижение, оптовая торговля)",
        "15*Детское питание (производство)",
        "15*Кондитерские изделия (продвижение, оптовая торговля)",
        "15*Кондитерские изделия (производство)",
        "15*Консервированная продукция (продвижение, оптовая торговля)",
        "15*Консервированная продукция (производство)",
        "15*Корма для животных (продвижение, оптовая торговля)",
        "15*Корма для животных (производство)",
        "15*Ликероводочная продукция (производство)",
        "15*Макаронные изделия (продвижение, оптовая торговля)",
        "15*Макаронные изделия (производство)",
        "15*Масложировая продукция (продвижение, оптовая торговля)",
        "15*Масложировая продукция (производство)",
        "15*Молочная продукция (продвижение, оптовая торговля)",
        "15*Молочная продукция (производство)",
        "15*Мука, крупы (продвижение, оптовая торговля)",
        "15*Мука, крупы (производство)",
        "15*Мясная продукция (продвижение, оптовая торговля)",
        "15*Мясная продукция (производство)",
        "15*Пиво (производство)",
        "15*Пищевые концентраты (продвижение, оптовая торговля)",
        "15*Пищевые концентраты (производство)",
        "15*Полуфабрикаты, в том числе замороженные  (продвижение, оптовая торговля)",
        "15*Полуфабрикаты, в том числе замороженные (производство)",
        "15*Рыба и морепродукты (продвижение, оптовая торговля)",
        "15*Рыба и морепродукты (производство)",
        "15*Сахар (продвижение, оптовая торговля)",
        "15*Сахар (производство)",
        "15*Свежие овощи, фрукты (продвижение, оптовая торговля)",
        "15*Снеки (продвижение, оптовая торговля)",
        "15*Снеки (производство)",
        "15*Хлебобулочные изделия (продвижение, оптовая торговля)",
        "15*Хлебобулочные изделия (производство)",
        "15*Чай, кофе (продвижение, оптовая торговля)",
        "15*Чай, кофе (производство)",
        "16*Дорожно-строительная техника, сельскохозяйственная и другая спец.техника, оборудование, лифты, погрузочно-разгрузочное, складское оборудование (монтаж, сервис, ремонт)",
        "16*Дорожно-строительная техника, сельскохозяйственная и другая спец.техника, оборудование, лифты, погрузочно-разгрузочное, складское оборудование (продвижение, оптовая торговля)",
        "16*Дорожно-строительная техника, сельскохозяйственная и другая спец.техника, оборудование, лифты, погрузочно-разгрузочное, складское оборудование (производство)",
        "16*Медицинское, диагностическое оборудование, инструменты (монтаж, сервис, ремонт)",
        "16*Медицинское, диагностическое оборудование, инструменты (продвижение, оптовая торговля)",
        "16*Медицинское, диагностическое оборудование, инструменты (производство)",
        "16*Оборудование для деревообработки, заготовки леса, целлюлозно-бумажной промышленности (монтаж, сервис, ремонт)",
        "16*Оборудование для деревообработки, заготовки леса, целлюлозно-бумажной промышленности (продвижение, оптовая торговля)",
        "16*Оборудование для деревообработки, заготовки леса, целлюлозно-бумажной промышленности (производство)",
        "16*Оборудование для пищевой промышленности, упаковки и хранения (монтаж, сервис, ремонт)",
        "16*Оборудование для пищевой промышленности, упаковки и хранения (продвижение, оптовая торговля)",
        "16*Оборудование для пищевой промышленности, упаковки и хранения (производство)",
        "16*Оборудование для ресторанов, гостиниц, кейтеринга, розничной торговли  (монтаж, сервис, ремонт)",
        "16*Оборудование для ресторанов, гостиниц, кейтеринга, розничной торговли  (продвижение, оптовая торговля)",
        "16*Оборудование для ресторанов, гостиниц, кейтеринга, розничной торговли (производство)",
        "16*Оборудование и станки для добывающей, энергетической, нефтегазовой и химической отрасли  (монтаж, сервис, ремонт)",
        "16*Оборудование и станки для добывающей, энергетической, нефтегазовой и химической отрасли  (продвижение, оптовая торговля)",
        "16*Оборудование и станки для добывающей, энергетической, нефтегазовой и химической отрасли  (производство)",
        "16*Оборудование и станки для металлургии и металлообработки (монтаж, сервис, ремонт)",
        "16*Оборудование и станки для металлургии и металлообработки (продвижение, оптовая торговля)",
        "16*Оборудование и станки для металлургии и металлообработки (производство)",
        "16*Оборудование и станки для отраслей легкой промышленности (монтаж, сервис, ремонт)",
        "16*Оборудование и станки для отраслей легкой промышленности (продвижение, оптовая торговля)",
        "16*Оборудование и станки для отраслей легкой промышленности (производство)",
        "17*Вендинг (торговля через автоматы)",
        "17*Зоомагазин",
        "17*Интернет-магазин",
        "17*Несетевая розница, мелкий опт",
        "17*Розничная сеть (DIY и товары для дома)",
        "17*Розничная сеть (drogerie, товары повседневного спроса)",
        "17*Розничная сеть (книги, игрушки, подарки, часы и прочее)",
        "17*Розничная сеть (мебель)",
        "17*Розничная сеть (мобильный ритейл)",
        "17*Розничная сеть (одежда, обувь, аксессуары)",
        "17*Розничная сеть (парфюмерия, косметика)",
        "17*Розничная сеть (продуктовая)",
        "17*Розничная сеть (спортивные товары)",
        "17*Розничная сеть (товары для детей)",
        "17*Розничная сеть (электроника, бытовая техника)",
        "17*Розничная сеть (ювелирные изделия)",
        "18*Издательская деятельность",
        "18*Киностудии и студии звукозаписи",
        "18*Маркетинговые, рекламные, BTL, дизайнерские, Event-, PR-агентства, организация выставок",
        "18*Продюсерский центр",
        "18*Производство и продажа рекламно-сувенирной продукции",
        "18*Производство мультимедиа, контента, редакторская деятельность",
        "18*Производство печатной, полиграфической продукции",
        "18*Распространение мультимедиа и печатной продукции",
        "18*Распространение телепрограмм, кино (кабельное телевидение)",
        "18*Теле- и радиовещание",
        "19*Земледелие, растениеводство, животноводство",
        "19*Рыболовство, рыбоводство",
        "19*Сельскохозяйственная продукция (продвижение, оптовая торговля)",
        "20*Агентские услуги в недвижимости",
        "20*Архитектура, проектирование",
        "20*Девелопмент",
        "20*Консалтинг, оценка в недвижимости",
        "20*Строительство дорожное и инфраструктурное",
        "20*Строительство жилищное",
        "20*Строительство коммерческих объектов (торговые площади, офисные здания)",
        "20*Строительство объектов металлургической отрасли",
        "20*Строительство объектов нефтегазовой отрасли",
        "20*Строительство объектов телекоммуникационной отрасли",
        "20*Строительство промышленное",
        "20*Строительство энергетическое",
        "20*Управление и эксплуатация недвижимости",
        "21*Мобильная связь",
        "21*Оптоволоконная связь",
        "21*Спутниковая связь",
        "21*Фиксированная связь",
        "22*Бытовая химия, парфюмерия, косметика  (продвижение, оптовая торговля)",
        "22*Бытовая химия, парфюмерия, косметика  (производство)",
        "22*Канцтовары, расходные материалы (продвижение, оптовая торговля)",
        "22*Канцтовары, расходные материалы (производство)",
        "22*Мебель (продвижение, оптовая торговля)",
        "22*Мебель (производство)",
        "22*Одежда, обувь, аксессуары, текстиль  (продвижение, оптовая торговля)",
        "22*Одежда, обувь, аксессуары, текстиль (производство)",
        "22*Посуда, свет, товары для дома  (продвижение, оптовая торговля)",
        "22*Посуда, свет, товары для дома  (производство)",
        "22*Сантехника (продвижение, оптовая торговля)",
        "22*Сантехника (производство)",
        "22*Спортивные товары и оборудование (продвижение, оптовая торговля)",
        "22*Спортивные товары и оборудование (производство)",
        "22*Стройматериалы, сборные конструкции для ремонта и строительства (продвижение, оптовая торговля)",
        "22*Стройматериалы, сборные конструкции для ремонта и строительства (производство, монтаж, сервис, ремонт)",
        "22*Табак  (продвижение, оптовая торговля)",
        "22*Табак  (производство)",
        "22*Товары для детей (продвижение, оптовая торговля)",
        "22*Товары для детей (производство)",
        "22*Ювелирные изделия, бижутерия  (продвижение, оптовая торговля)",
        "22*Ювелирные изделия, бижутерия  (производство)",
        "23*Авиационная, вертолетная, аэрокосмическая промышленность",
        "23*Двигатели (монтаж, сервис, ремонт)",
        "23*Двигатели (продвижение, оптовая торговля)",
        "23*Двигатели (производство)",
        "23*Железнодорожное машиностроение, вагоностроение",
        "23*Продукция авиационной, аэрокосмической, вертолетной промышленности (монтаж, сервис, ремонт)",
        "23*Продукция авиационной, аэрокосмической, вертолетной промышленности (продвижение, оптовая торговля)",
        "23*Продукция железнодорожного машиностроения, вагоностроения (монтаж, сервис, ремонт)",
        "23*Продукция железнодорожного машиностроения, вагоностроения (продвижение, оптовая торговля)",
        "23*Продукция судостроения (продвижение, оптовая торговля)",
        "23*Производство и ремонт военной техники, оборудования и вооружения",
        "23*Судоремонт",
        "23*Судостроение",
        "24,Управляющая компания группы, холдинга, штаб-квартира",
        "25*Безопасность, охранная деятельность",
        "25*Кадровые агентства",
        "25*Колл-центры, диспетчерские службы",
        "25*Консалтинговые услуги",
        "25*Контроль качества, экспертиза, испытания и сертификация",
        "25*Переводческая деятельность",
        "25*Услуги по организации поездок и бронированию",
        "25*Экологическое обслуживание, водоочистка, утилизация отходов",
        "25*Юридическое обслуживание",
        "26*Аренда автомобилей",
        "26*Домашний персонал",
        "26*Игорный бизнес",
        "26*Пошив, ремонт одежды и обуви",
        "26*Ритуальные услуги",
        "26*Салоны красоты",
        "26*Спортивные и фитнес-клубы",
        "26*Такси",
        "26*Туристические компании",
        "26*Фото- и видеоуслуги",
        "26*Химчистка",
        "26*Центры развлечения",
        "27*Аудит, управленческий учет, финансово-юридический консалтинг",
        "27*Банк",
        "27*Коллекторская деятельность",
        "27*Лизинговые компании",
        "27*НПФ",
        "27*Страхование, перестрахование",
        "27*Управляющая, инвестиционная компания (управление активами)",
        "27*Услуги по ведению бухгалтерского и налогового учета, расчет заработной платы",
        "27*Факторинговые компании",
        "27*Финансово-кредитное посредничество (биржа, брокерская деятельность, выпуск и обслуживание карт, оценка рисков, обменные пункты, агентства по кредитованию, инкассация, ломбард, платежные системы)",
        "28*Агрохимия (продвижение, оптовая торговля)",
        "28*Агрохимия (производство)",
        "28*Бетон, кирпич, стекло и прочие силикаты (продвижение, оптовая торговля)",
        "28*Бетон, кирпич, стекло и прочие силикаты (производство)",
        "28*Лакокрасочная продукция, сырье (продвижение, оптовая торговля)",
        "28*Лакокрасочная продукция, сырье (производство)",
        "28*Неорганическая химия (продвижение, оптовая торговля)",
        "28*Неорганическая химия (производство)",
        "28*Органическая химия (продвижение, оптовая торговля)",
        "28*Органическая химия (производство)",
        "28*Резина, пластмасса и прочее (продвижение, оптовая торговля)",
        "28*Резина, пластмасса и прочее (производство)",
        "29*Бытовая техника, электроника, климатическое оборудование (монтаж, сервис, ремонт)",
        "29*Бытовая техника, электроника, климатическое оборудование (продвижение, оптовая торговля)",
        "29*Бытовая техника, электроника, климатическое оборудование (производство)",
        "29*Промышленное, бытовое электрооборудование и электротехника (монтаж, сервис, ремонт)",
        "29*Промышленное, бытовое электрооборудование и электротехника (продвижение, оптовая торговля)",
        "29*Промышленное, бытовое электрооборудование и электротехника (производство)",
        "29*Электронно-вычислительная, оптическая, контрольно-измерительная техника, радиоэлектроника, автоматика (монтаж, сервис, ремонт)",
        "29*Электронно-вычислительная, оптическая, контрольно-измерительная техника, радиоэлектроника, автоматика (продвижение, оптовая торговля)",
        "29*Электронно-вычислительная, оптическая, контрольно-измерительная техника, радиоэлектроника, автоматика (производство)",
        "30*Альтернативная энергетика: гео-, ветро-, солнце-, био- (генерация электроэнергии)",
        "30*Атомная энергетика (генерация электроэнергии, АЭС)",
        "30*Гидроэнергетика (генерация электроэнергии, ГЭС)",
        "30*Инфраструктурная компания в энергетике (СО ЕЭС, АТС, ЦФР и т.п.)",
        "30*Консалтинг в области энергоэффективности",
        "30*Передача, распределение электроэнергии",
        "30*Сбыт и потребление электроэнергии",
        "30*Теплоснабжение (ТЭЦ)",
        "30*Теплоэнергетика (генерация электроэнергии и тепла, ТЭС)",
    );

    foreach ($tags as $tag_val) {
        $tag = explode('*', $tag_val);

        $db->insert(T_USER_CATEGOGIES_TAGS, [
            'category_id' => $tag[0],
            'name' => $tag[1]
        ]);
    }
...
//ПОЛУЧАЕМ СФЕРУ ДЕЯТЕЛЬНОСТИ
function Wo_GetUserCategories($category_id = null) {
    global $wo, $db;

    if ($category_id) {
        $db->where("id", (int) $category_id);
    }
...
//ПОЛУЧАЕМ ГороД
function Wo_GetUserCity($city_id = null) {
    global $wo, $db;
    ...
    if ($city_id) {
        $db->where("id", (int) $city_id);
    }

    return $db->get(T_MY_CITY);
}

//ПОЛУЧАЕМ СПЕЦИАЛИЗАЦИЮ ПО КАТЕГОРИИ, ВЫВОД В ФОРМАТЕ json/array
function Wo_GetUserCategoryTags($result_type = 'json', $category_id = null) {
    global $wo, $db;

    if ($category_id) {
        $db->where("category_id", (int) $category_id);
    }

    if ($result_type == 'array') {
        return $db->get(T_USER_CATEGOGIES_TAGS);
    }

    return json_encode($db->get(T_USER_CATEGOGIES_TAGS), JSON_UNESCAPED_UNICODE);
}

//ДОБАВЛЯЕМ СПЕЦИАЛИЗАЦИЮ ПОЛЬЗОВАТЕЛЯ
function Wo_AddUserCategoryTags($user_id, $category_id, $tags = []) {
    global $wo, $db;

    if (is_array($tags) && count($tags)) {
        foreach ($tags as $tag) {
            $db->insert(T_USER_TAGS, [
        ...
        $tags = $db->where("category_id", (int) $category_id)->get(T_USER_CATEGOGIES_TAGS);
        foreach ($tags as $tag) {
            $db->insert(T_USER_TAGS, [
                'category_id' => (int) $category_id,
                'tag_id' => $tag->id,
...
//ОБНОВЛЯЕМ СПЕЦИАЛИЗАЦИЮ ПОЛЬЗОВАТЕЛЯ
function Wo_UpadeteUserCategoryTags($user_id, $category_id, $tags = []) {
    global $wo, $db;

    $db->where("user_id", (int) $user_id)->delete(T_USER_TAGS);

    Wo_AddUserCategoryTags($user_id, $category_id, $tags);
}

//ПОЛУЧАЕМ ДАННЫЕ О СПЕЦИАЛИЗАЦИИ ПОЛЬЗОВАТЕЛЯ
function Wo_GetUserProfileTags($user_id, $type = 'json') {
    global $wo, $db;
    $arr_tag = [];
    $tags = $db->where("user_id", (int) $user_id)->get(T_USER_TAGS);
    if (count($tags)) {
        foreach ($tags as $tag) {
            $arr_tag[$tag->tag_id] = $tag->tag_id;
        }
    }
    if ($type == 'array') {
        $tags_arr = array();
        foreach ($arr_tag as $value) {
            $tags_arr[] = $db->where("id", (int) $value)->getOne(T_USER_CATEGOGIES_TAGS);
        }

        return $tags_arr;
    }
    return json_encode($arr_tag, JSON_UNESCAPED_UNICODE);
}

//ПОЛУЧАЕМ ДАННЫЕ О ЗАПРОСЕ НА ПОИСК ПОЛЬЗОВАТЕЛЕЙ ПО СПЕЦИАЛИЗАЦИИ
function Wo_GetUserFindTags($user_id, $type = 'json') {
    global $wo, $db;
    $arr_tag = [];
    $tags = $db->where("user_id", (int) $user_id)->get(T_USER_FIND_TAGS);
    if (count($tags)) {
        foreach ($tags as $tag) {
            $arr_tag[$tag->tag_id] = $tag->tag_id;
        }
...
//ПОЛУЧАЕМ ДАННЫЕ О КОЛИЧЕСТВЕ ЗАПРОСА НА ПОИСК ПОЛЬЗОВАТЕЛЕЙ ПО СПЕЦИАЛИЗАЦИИ
function Wo_GetUserFindTagsCount($user_id) {
    global $wo, $db;
    $tags = $db->where("user_id", (int) $user_id)->get(T_USER_FIND_TAGS);
    return count($tags);
}

//ПОЛУЧАЕМ ДАННЫЕ О КОЛИЧЕСТВЕ СПЕЦИАЛИЗАЦИЙ ПОЛЬЗОВАТЕЛЯ
function Wo_GetUserTagsCount($user_id) {
    global $wo, $db;
    $tags = $db->where("user_id", (int) $user_id)->get(T_USER_TAGS);
    return count($tags);
}

//ДОБАВЛЯЕМ (И ОБНОВЛЯЕМ) НОВЫЕ ДАННЫЕ В ПОИСК ПОЛЬЗОВАТЕЛЯ ПО СПЕЦИАЛИЗАЦИИ
function Wo_AddUserFindTags($user_id, $tags = []) {
    global $wo, $db;

    $db->where("user_id", (int) $user_id)->delete(T_USER_FIND_TAGS);

    if (count($tags)) {
        foreach ($tags as $tag) {

            $db->insert(T_USER_FIND_TAGS, [
                'tag_id' => $tag,
                'user_id' => $user_id
            ]);
        }
...
function Wo_GetCity($city_id = null) {
    global $wo, $db;
...
//JWT GENERATE
function Wo_JWT_Generate($user_id) {
    global $wo, $db;
    ...
    $user = $db->where("user_id", (int) $user_id)->getOne(T_USERS);
    ...
    $payload = [
        ...
        "aud" => "jitsi",
        "iss" => "rjMOpakXngyeFhKYgHxQDLRRaySTDq4ZZqEUZtq3M6AlA13k0W",
        "sub" => "event",
        "room" => "*",
        "moderator" => true,
        "nbf" => time(),
    ...
    $key = "lpttfj9A4ih5OxoxCNAA87hr3jvw7LNYGeuJhXWtG4S4Vmqvmj";
...
//ДОП ПАСЕР ДЛЯ ФУНКЦИИ ГЕНЕРАЦИИ JWT
...
//СОЗДАЕМ МАССИВ СТОИМОСТИ СОЗДАНИЯ МЕРОПРИЯТИЯ
function EventsCoastsTime($id = null, $base_coast = 250, $event_coast = 1000) {
    $arr = array();
    for ($i = 1; $i < 12; $i++) {
        $arr[$i] = $event_coast + ($base_coast * ($i - 1));
    }
    if ($id) {
        return $arr[$id];
    }
...
//ПРОВЕРЯЕМ ЭТОТ ЧАТ ОТНОСИТСЯ К МЕРОПРИТИЮ ИЛИ НЕТ
function IsEventChatGroup($chat_id) {
    global $db;
    $group = $db->where("group_id", $chat_id)->getOne(T_GROUP_CHAT);

    return ((bool) $group->event_id);
}

//ЗАКРЫВАЕМ ЧАТ МЕРОПРИЯТИЯ
function Wo_CloseEventChatGroup($chat_id) {
    global $db;
    $group = $db->where("group_id", $chat_id)->getOne(T_GROUP_CHAT);
    if ($group->close) {
        $db->where("group_id", $chat_id)->update(T_GROUP_CHAT, array(
            'close' => 0
        ));
        return true;
    }
...
//ПРОВЕРЯЕМ ОТНОСИТСЯ ЛИ ЧАТ К МЕРОПРИЯТИЮ
function GetIdGroupChat($event_id) {
    global $db;
    $group = $db->where("event_id", $event_id)->getOne(T_GROUP_CHAT);
    return $group->group_id;
}

//ПРОВЕРЯЕМ ОТКРЫТ ЛИ ЧАТ
function Is_GroupChatClosed($chat_id) {
    global $db;
    $group = $db->where("group_id", $chat_id)->getOne(T_GROUP_CHAT);
    return (bool) (int) $group->close;
}

//СЧИТАЕМ СОБРАННЫЕ СТРЕДСТВА НА МЕРОПРИЯТИЯ
function GetEventPayment($event_id = null) {
    global $wo, $db;

    $g_users = $db->where("event_id", $event_id)
            ->where("egoing_user_id", 0, "<>")
            ->where("upadated_at", 0)
            ->get(T_SYS_WALLET);

    $sum_event['sum'] = $db->where('event_id', $event_id)->where("egoing_user_id", 0, "<>")->getValue(T_SYS_WALLET, "SUM(balance)");
    $sum_event['count'] = $db->where('event_id', $event_id)->where("egoing_user_id", 0, "<>")->where("upadated_at", 0)->getValue(T_SYS_WALLET, "COUNT(balance)");
    return $sum_event;
}

//БАЛЛАНС СИСТЕМЫ
function Wo_AddPayForSysWallet($event_id, $type = 'event_create', $pay = 0, $cf_percent = 0) {
    global $wo, $db;

    if ($type === 'event_create') {
        
       
        //XXX СНИМАТЬ ДЕНЬГИ СО СЧЕТА ПОЛЬЗОВАТЕЛЯ
        $db->insert(T_SYS_WALLET, array(
            'event_id' => $event_id,
            'event_poster_id' => $wo['user']['id'],
            'pay_in' => $pay,
            'balance' => $pay,
            'created_at' => time(),
        ));

        //TODO ДОБАВЛЯЕМ ДАННЫЕ В ТРАНЗАКЦИИ
        $notes = json_encode([
            'event_id' => $event_id,
            'method_type' => 'wallet'
        ]);

        $db->insert(T_PAYMENT_TRANSACTIONS, array(
            "userid" => $wo['user']['id'],
            "kind" => "EVENT_CREATE_OR_UPDATE",
            "amount" => $pay ?? 0,
            "notes" => $notes,
            'event_id' => $event_id,
        ));
          //XXX СНИМАТЬ ИЛИ ВОЗВРАЩАТЬ ДЕНЬГИ НА СЧЕТА ПОЛЬЗОВАТЕЛЯ
        $db->where("user_id", $wo['user']['id'])
                ->update(T_USERS, array(
                    'wallet' => (int)$wo['user']['wallet'] - (int)$pay,
        ));
        //TODO ДОБАВЛЯЕМ УВЕДОМЛЕНИЕ О СНЯТИИ СРЕДСТВ
        $notification_data_array = array(
            'notifier_id' => 1,
            'recipient_id' => $wo['user']['id'],
            'type' => 'sent_u_money',
            'user_id' => $wo['user']['id'],
            'text' => "СОЗДАНИЕ МЕРОПРИЯТИЯ. С Вашего счета снято $pay рублей!",
    ...
    if ($type === 'event_update') { 
        ...
        $ev_sys = $db->where("event_id", $event_id)
                ->where("event_poster_id", $wo['user']['id'])
                ->getOne(T_SYS_WALLET);
        
        $user_wallet = ($wo['user']['wallet'] + $ev_sys->pay_in) - (int)$pay;
        ...
        $db->where("kind", "EVENT_CREATE_OR_UPDATE")
                ->where("event_id", $event_id)
                ->update(T_PAYMENT_TRANSACTIONS, array(
                    "userid" => $wo['user']['id'],
                    "kind" => "EVENT_CREATE_OR_UPDATE",
                    "amount" => $pay ?? 0,
                    "notes" => $notes,
        ...
        $notification_data_array = array(
            ...
            'type' => 'sent_u_money',
            'user_id' => $wo['user']['id'],
            'text' => "ИЗМЕНЕНИЯ МЕРОПРИЯТИЯ. ВОЗВРАТ. На Ваш счет возвращено $ev_sys->pay_in рублей. СНЯТИЕ. С Вашего счета снято $pay рублей!",
            'url' => 'index.php?link1=wallet'
        );
        Wo_RegisterNotification($notification_data_array);
    }

    if ($type === 'event_going_in') {
        //XXX СНИМАТЬ ИЛИ ВОЗВРАЩАТЬ ДЕНЬГИ НА СЧЕТА ПОЛЬЗОВАТЕЛЯ
        $db->insert(T_SYS_WALLET, array(
            'event_id' => $event_id,
            'egoing_user_id' => $wo['user']['id'],
            'pay_in' => $pay,
            'balance' => $pay,
            'created_at' => time(),
        ));
        ...
        $notification_data_array = array(
            ...
            'type' => 'sent_u_money',
            'user_id' => $wo['user']['id'],
            'text' => "ВЫ ИДЁТЕ НА МЕРОПРИЯТИЕ. С Вашего счета снято $pay рублей!",
            'url' => 'index.php?link1=wallet'
        );
        Wo_RegisterNotification($notification_data_array);
    }

    if ($type === 'event_going_out') {

        //XXX СНИМАТЬ ИЛИ ВОЗВРАЩАТЬ ДЕНЬГИ НА СЧЕТА ПОЛЬЗОВАТЕЛЯ
        $db->where("event_id", $event_id)
                ->where("egoing_user_id", $wo['user']['id'])
                ->where("upadated_at", 0)
                ->update(T_SYS_WALLET, array(
                    'pay_out' => ($pay * $cf_percent),
                    'pay_percent' => $cf_percent * 100,
                    'balance' => $pay - ($pay * $cf_percent),
                    'upadated_at' => time(),
        ));
        ...
        $db->insert(T_PAYMENT_TRANSACTIONS, array(
            "userid" => $wo['user']['id'],
            "kind" => "SYS_EVENT_GOING_OUT",
            "amount" => $pay - ($pay * $cf_percent),
            "notes" => $notes,
            'event_id' => $event_id,
        ));
        ...
        $db->insert(T_PAYMENT_TRANSACTIONS, array(
            "userid" => $wo['user']['id'],
            "kind" => "EVENT_GOING_OUT",
            "amount" => ($pay * $cf_percent),
            "notes" => $notes,
            'event_id' => $event_id,
        ));
        //TODO ДОБАВЛЯЕМ УВЕДОМЛЕНИЕ О СНЯТИИ СРЕДСТВ
        $notification_data_array = array(
            ...
            'type' => 'sent_u_money',
            'user_id' => $wo['user']['id'],
            'text' => "ВЫ ВЫШЛИ ИЗ МЕРОПРИЯТИЯ. На Ваш счет возвращено " . ($pay * $cf_percent) . " рублей!",
            'url' => 'index.php?link1=wallet'
        );
        Wo_RegisterNotification($notification_data_array);
    }
    ...
    if ($type === 'event_close') {
        ...
        //XXX ВОЗВРАЩАЕМ ДЕНЬГИ ВСЕМ УЧАСТНИКАМ МЕРОПРИЯТИЯ
        $g_users = $db->where("event_id", $event_id)
                ->where("egoing_user_id", 0, "<>")
                ->where("upadated_at", 0)
                ->get(T_SYS_WALLET);
        ...
        foreach ($g_users as $g_usr) {
            //ВОЗВРАЩАЕМ ДЕНЬГИ ВСЕМ УЧАСТНИКАМ МЕРОПРИЯТИЯ
            $db->where("event_id", $event_id)
                    ->where("egoing_user_id", $g_usr->egoing_user_id)
                    ->where("upadated_at", 0)
                    ->update(T_SYS_WALLET, array(
                        'pay_out' => $g_usr->pay_in,
                        'pay_percent' => 0,
                        'balance' => 0,
                        'upadated_at' => time(),
                        'cash_date' => time(),
            ));
            //ДОБАВЛЯЕМ ДАННЫЕ О ВОЗВРАТЕ ДЕНЕГ ЗА МЕРОПРИЯТИЕ
            $db->insert(T_PAYMENT_TRANSACTIONS, array(
                "userid" => $g_usr->egoing_user_id,
                "kind" => "EVENT_GOING_OUT",
                "amount" => $g_usr->pay_in,
                "notes" => $notes,
                'event_id' => $event_id,
            ));
            //TODO ДОБАВЛЯЕМ УВЕДОМЛЕНИЕ О ВОЗВРАТЕ СРЕДСТВ
            $notification_data_array = array(
                'notifier_id' => 1,
                'recipient_id' => $g_usr->egoing_user_id,
                'type' => 'sent_u_money',
                'user_id' => $g_usr->egoing_user_id,
                'text' => "МЕРОПРИЯТИЕ ОТМЕНЕНО. На Ваш счет возвращено " . ($g_usr->pay_in) . " рублей!",
                'url' => 'index.php?link1=wallet'
            );
            Wo_RegisterNotification($notification_data_array);
        }
        //XXX ПЕРЕВОДИМ ДЕНЬГИ НА СИСТЕМНЫЙ СЧЕТ ТЕХ КТО ВЫШЕЛ ИЗ МЕРОПРИЯТИЯ
        $g_out_users = $db->where("event_id", $event_id)
                ->where("egoing_user_id", 0, "<>")
                ->get(T_SYS_WALLET);
//ДЕНЬГИ ТЕМ КТО ВЫШЕЛ ИЗ МЕРОПРИЯТИЕ НЕ ВОЗВРАЩАЮТСЯ
        foreach ($g_out_users as $g_usr) {
            $db->where("event_id", $event_id)
                    ->where("egoing_user_id", $g_usr->egoing_user_id)
                    ->where("balance", 0, "<>")
                    ->update(T_SYS_WALLET, array(
                        'balance' => 0,
                        'sys_balance' => $g_usr->balance,
                        'upadated_at' => time(),
                        'cash_date' => time(),
            ));
            //ВНОСИМ ДАННЫЕ О ТРАНЗАКЦИИ
            $db->insert(T_PAYMENT_TRANSACTIONS, array(
                "userid" => $g_usr->egoing_user_id,
                "kind" => "SYS_EVENT_GOING_OUT",
                "amount" => $g_usr->balance,
                "notes" => $notes,
                'event_id' => $event_id,
            ));
        }
        //XXX СНИМАТЬ И ВОЗВРАЩАТЬ ДЕНЬГИ НА СЧЕТА СОЗДАТЕЛЯ МЕРОПРИЯТИЯ
        $sys_bal = $db->where("event_id", $event_id)
                ->where("event_poster_id", $wo['user']['id'])
                ->getOne(T_SYS_WALLET);

        $pay = $sys_bal->pay_in;
//ВОЗВРАТ ДЕНЕГ ОРГАНИЗАТОРУ
        $db->where("event_id", $event_id)
                ->where("event_poster_id", $wo['user']['id'])
                ->update(T_SYS_WALLET, array(
                    'pay_out' => ($pay * $cf_percent),
                    'pay_poster' => ($pay * $cf_percent),
                    'pay_percent' => $cf_percent * 100,
                    'balance' => 0,
                    'sys_balance' => $pay - ($pay * $cf_percent),
                    'upadated_at' => time(),
                    'cash_date' => time(),
        ));
        //ВНОСИМ ДАННЫЕ О ВОЗВРАТЕ СРЕДСТВ ОРГАНИЗАТОРУ
        $db->insert(T_PAYMENT_TRANSACTIONS, array(
            "userid" => $wo['user']['id'],
            "kind" => "EVENT_CLOSE",
            "amount" => ($pay * $cf_percent),
        ...
        //ДАННЫЕ В СИСТЕМНЫЙ КОШЕЛЕК
        $db->insert(T_PAYMENT_TRANSACTIONS, array(
            "userid" => $wo['user']['id'],
            "kind" => "SYS_EVENT_CLOSE",
            "amount" => $pay - ($pay * $cf_percent),
            "notes" => $notes,
            'event_id' => $event_id,
        ));
        ...
        $notification_data_array = array(
            ...
            'type' => 'sent_u_money',
            'user_id' => $wo['user']['id'],
            'text' => "МЕРОПРИЯТИЕ ОТМЕНЕНО. На Ваш счет возвращено " . ($pay * $cf_percent) . " рублей!",
            'url' => 'index.php?link1=wallet'
        );
        Wo_RegisterNotification($notification_data_array);
    }

    //TODO ОКОНЧАНИЕ МЕРОПРИЯТИЯ - ВЫПЛАТА ДЕНЕГ
    if ($type === 'event_end') {
        $all_pay = 0;
        $event = $db->where("id", $event_id)->getOne(T_EVENTS);

        //XXX РАСПРЕДЕЛЯЕМ ДЕНЬГИ УЧАСТНИКОВ МЕЖДУ СИСТЕМОЙ И СОЗДАТЕЛЕМ МЕРОПРИЯТИЯ
        $g_users = $db->where("event_id", $event_id)
                ->where("egoing_user_id", 0, "<>")
                ->where("upadated_at", 0)
                ->get(T_SYS_WALLET);
        ...
        foreach ($g_users as $g_usr) {
            $db->where("event_id", $event_id)
                    ...
                    ->where("upadated_at", 0)
                    ->update(T_SYS_WALLET, array(
                        'pay_poster' => ((int) ($g_usr->pay_in) * $cf_percent),
                        'pay_percent' => $cf_percent * 100,
                        'balance' => 0,
                        'sys_balance' => (int) ($g_usr->pay_in) - ((int) ($g_usr->pay_in) * $cf_percent),
                        'upadated_at' => time(),
                        'cash_date' => time(),
            ));

            $all_pay = $all_pay + ((int) ($g_usr->pay_in) * $cf_percent);
            //TODO ДОБАВЛЯЕМ ДЕНЬГИ НА СЧЕТ СОЗДАТЕЛЯ МЕРОПРИЯТИЯ
            $user = $db->where("user_id", $event->poster_id)->getOne(T_USERS);
            $db->where("user_id", $event->poster_id)->update(T_USERS, array(
                'wallet' => (int) ($user->wallet) + ((int) ($g_usr->pay_in) * $cf_percent)
            ));
            ...
            $db->insert(T_PAYMENT_TRANSACTIONS, array(
                "userid" => $wo['user']['id'],
                "kind" => "EVENT_GOING_END",
                "amount" => ((int) ($g_usr->pay_in) * $cf_percent),
                "notes" => $notes,
                'event_id' => $event_id,
            ));
            //ДАННЫЕ В СИСТЕМНЫЙ КОШЕЛЕК
            $db->insert(T_PAYMENT_TRANSACTIONS, array(
                "userid" => $wo['user']['id'],
                "kind" => "SYS_EVENT_GOING_END",
                "amount" => (int) ($g_usr->pay_in) - ((int) ($g_usr->pay_in) * $cf_percent),
                "notes" => $notes,
                'event_id' => $event_id,
            ));
        }

        $sys_wal_ev = $db->where("event_id", $event_id)
                ->where("event_poster_id", $event->poster_id)
                ->getOne(T_SYS_WALLET);
        //TODO ПЕРЕВОДИМ ДЕНЬГИ НА СИСТЕМНЫЙ СЧЕТ СО СЧЕТА СОЗДАТЕЛЯ, ЕСЛИ МЕРОПРИЯТИЕ ОКОНЧЕНО
        $db->where("event_id", $event_id)
                ->where("event_poster_id", $event->poster_id)
                ->update(T_SYS_WALLET, array(
                    'pay_out' => 0,
                    'pay_percent' => 0,
                    'balance' => 0,
                    'sys_balance' => $sys_wal_ev->balance,
                    'upadated_at' => time(),
                    'cash_date' => time(),
        ));
        //TODO ДОБАВЛЯЕМ ДАННЫЕ В ТРАНЗАКЦИИ
        ...
        //ВНОСИМ ДАННЫЕ О ПЕРЕВОДЕ ДЕНЕГ СИСТЕМЕ
        $db->insert(T_PAYMENT_TRANSACTIONS, array(
            "userid" => $wo['user']['id'],
            "kind" => "EVENT_END",
            "amount" => $sys_wal_ev->balance,
            "notes" => $notes,
            'event_id' => $event_id,
        ));
        //XXX ПЕРЕВОДИМ ДЕНЬГИ НА СИСТЕМНЫЙ СЧЕТ ТЕХ КТО ВЫШЕЛ ИЗ МЕРОПРИЯТИЯ
        ...
        //УВЕДОМЛЕНИЕ О ЗАЧИСЛЕНИИ СРЕДСТВ ОРГАНИЗАТОРУ
        $notification_data_array = array(
            'notifier_id' => 1,
            'recipient_id' => $wo['user']['id'],
            'type' => 'sent_u_money',
            'user_id' => $wo['user']['id'],
            'text' => "ВЫ ПРОВЕЛИ МЕРОПРИЯТИЕ. На Ваш счет зачислено " . $all_pay . " рублей!",
            'url' => 'index.php?link1=wallet'
        );
        Wo_RegisterNotification($notification_data_array);
    }
...
function Get_Rus_Mounth($date) {
    ...
    return mb_substr($mounth[$d_arr[1] - 1], 0, 3);
}

function Wo_EventSendMail($event_id = null, $message = '') {
    global $wo, $db;

    $event = $db->where("id", $event_id)->getOne(T_EVENTS);

    $u_goings = $db->where("event_id", $event_id)->get(T_EVENTS_GOING);

    if (count($u_goings)) {
        foreach ($u_goings as $g_user) {
            $user = $db->where("user_id", $g_user->user_id)->getOne(T_USERS);
            ...
            return Wo_SendMessage($send_message_data);
        }
    }
}

function Is_ActivationForSearch() {
    global $wo, $db;
    ...
    if (empty($wo['user']['first_name']) || empty($wo['user']['last_name']) || empty($wo['user']['city_id']) || empty($wo['user']['category_id']) || count(Wo_GetUserFindTags($wo['user']['id'], 'array')) == 0) {
        return true;
    }
...
function Get_VerificationDoc($user_id = null) {
    global $wo, $db;

    return $db->where("user_id", $wo['user']['id'])->getOne(T_VERIFICATION_REQUESTS);
}

function Wo_DelVerificationDoc($ver_id) {
    global $wo, $db;

    return $db->where("id", $ver_id)->delete(T_VERIFICATION_REQUESTS);
}

function getOrderRNCB($order_id) {
    $username = 'TerminalSys/50000308';
    $password = 'Nov@22112023';
    $data = array();

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://mpi.rncb.ru:8000/order/" . $order_id . "?orderDetailLevel=1");
    ...
    $data['id'] = $output->order->id;
    $data['amount'] = $output->order->amount;
    $data['status'] = $output->order->status;
...
function Wo_AddMoneyForUserWallet($sum, $order_id) {
    global $wo, $db;

    
      $check_tranzaction = $db->where("notes", "RNCB|".$order_id)->getOne(T_PAYMENT_TRANSACTIONS);
      
      if($check_tranzaction) {
          return false;
      }
    
    $db->where("user_id", $wo['user']['id'])
            ->update(T_USERS, array(
                'wallet' => $wo['user']['wallet'] + $sum
    ));
...
//ВНОСИМ ДАННЫЕ О ТРАНЗАКЦИИ
    ...
    //TODO ДОБАВЛЯЕМ УВЕДОМЛЕНИЕ О СНЕСЕНИИ СРЕДСТВ НА СЧЕТ
    $notification_data_array = array(
        'notifier_id' => 1,
        'recipient_id' => $wo['user']['id'],
        'type' => 'sent_u_money',
        'user_id' => $wo['user']['id'],
        'text' => "Ваш кошелек пополнен на " . $sum . " рублей",
        'url' => 'index.php?link1=wallet'
    );
...
function Wo_CheckVerifcateFiles($file_name){
    ...
    return in_array($info->getExtension(), $img_type, true);
}
